...
def validate_main_rank(rank: int | None) -> None:
    ...
    if isinstance(rank, bool) or not isinstance(rank, int) or rank < MAIN_RANK_MIN or rank > MAIN_RANK_MAX:
        raise HTTPException(
            status_code=422,
            detail=(
...
def validate_advanced_rank(rank: int | None) -> None:
    ...
    if isinstance(rank, bool) or not isinstance(rank, int) or rank < ADVANCED_RANK_MIN or rank > ADVANCED_RANK_MAX:
        raise HTTPException(
            status_code=422,
            detail=(
...
def validate_quotas(quotas: list[str] | None) -> None:
    """Raise 422 if any requested quota is unsupported."""
    if not quotas:
        return
    invalid = [quota for quota in quotas if quota not in VALID_QUOTAS]
    if invalid:
        raise HTTPException(
            status_code=422,
            detail=(
                f"Unknown quota(s): {invalid}. "
                f"Valid values: {sorted(VALID_QUOTAS)}."
            ),
        )
...
def validate_predict_request(request: object) -> None:
    ...
    validate_quotas(getattr(request, "pref_quotas", None))
