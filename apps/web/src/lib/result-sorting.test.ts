...
const supportedQuotas = ["AI", "HS", "OS", "STATE"] as const satisfies readonly QuotaApplied[];
...
test("highest likelihood estimate is explicit opt-in", () => {
  const sorted = sortResults(
    [
      result("safe", 99, 20, 80_000),
      result("fit", 62, 88, 24_000),
...
test("all expected sort labels are present", () => {
  ...
  assert.equal(RESULT_SORT_LABELS.highest_probability, "Highest likelihood estimate");
});

test("frontend contract accepts every backend-supported quota", () => {
  assert.deepEqual(supportedQuotas, ["AI", "HS", "OS", "STATE"]);
});
