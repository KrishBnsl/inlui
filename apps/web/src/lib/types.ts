...
export type QuotaApplied = "AI" | "HS" | "OS" | "STATE";
...
export interface SimulationResponse {
  ...
  // Research-transparency metadata reported by the inference service.
  // Null means the active artifact did not publish that value; the UI must
  ...
  returned_bucket_counts?: Record<RecommendationBucket, number>;
...
export function getRankTypeLabel(result: PredictionResult): string {
  if (
    result.rank_type_used === "JEE_ADVANCED" ||
    result.rank_type_used === "advanced"
  ) {
    return "JEE Advanced";
  }
  if (result.rank_type_used === "JEE_MAIN" || result.rank_type_used === "main") {
    return "JEE Main";
  }

  // JoSAA routing is deterministic: IIT rows use Advanced rank and all other
  // institute types use Main rank. Keep the fallback explicit in the label.
  return result.institute_type === "IIT"
    ? "JEE Advanced (institute rule)"
    : "JEE Main (institute rule)";
}
