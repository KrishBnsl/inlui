...
interface Message {
  ...
  sources?: Source[];      // retrieved source metadata/excerpts shown beside the answer
  error?: boolean;
}
...
type AdvisorHealth = "checking" | "ready" | "degraded" | "offline";
        ...
        {open && (
          ...
          >
            <div className="divide-y divide-neutral-700/40">
              {sources.map((s, i) => (
                <div key={`${s.source}-${s.excerpt}-${i}`} className="px-3 py-2 bg-neutral-900/40">
...
export default function ChatAdvisor({ open, onClose, recommendationData }: ChatAdvisorProps) {
  ...
  const [healthStatus, setHealthStatus] = useState<AdvisorHealth>("checking");
  const [healthMessage, setHealthMessage] = useState("Checking Gemini and RAG health…");
  ...
  useEffect(() => {
    ...
    setHealthMessage("Checking Gemini and RAG health…");
    getRagHealth()
      .then((health) => {
        if (!active) return;
        if (health.google_api_key_configured && health.status === "ok") {
          setHealthStatus("ready");
          setHealthMessage(
            health.chunk_count > 0
              ? `Gemini configured · ${health.chunk_count} source chunks indexed`
              : "Gemini configured · no document sources indexed yet"
          );
        } else {
          setHealthStatus("degraded");
          const reason = !health.google_api_key_configured
            ? "GOOGLE_API_KEY is not configured"
            : health.degraded_reasons?.[0] ?? "one or more RAG providers are unavailable";
          setHealthMessage(`RAG degraded: ${reason}.`);
        }
      })
      .catch(() => {
        if (active) {
          setHealthStatus("offline");
          setHealthMessage("RAG service unavailable.");
        }
      });
    return () => {
      active = false;
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: uid(),
            role: "assistant",
            text: `Indexed ${result.source}: ${result.chunks_added} source chunk${result.chunks_added !== 1 ? "s" : ""} added. You can now ask questions about it.`,
          },
        ]);
      } catch (e) {
    ...
    try {
      ...
      if (recommendationData) {
        try {
          ...
          setContextWarning(
            "Recommendation context is unavailable; the answer uses RAG sources only."
          );
        }
      }
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p id="advisor-title" className="text-sm font-semibold text-white leading-tight">
                    Counselling Advisor
                  </p>
              ...
              <button
                ...
                type="button"
                onClick={onClose}
                aria-label="Close counselling advisor"
            ...
            {attachedImage && (
              <div className="mx-4 mb-2 flex items-center gap-2 px-3 py-2 rounded-lg bg-neutral-800/70 border border-neutral-700/50">
                ...
                <button
                  ...
                  aria-label={`Remove attached image ${attachedImage.name}`}
            ...
            {/* ── Upload status / error ───────────────────────────────────────── */}
            {(uploading || uploadError || contextWarning) && (
              <div className="mx-4 mb-2">
                {uploading ? (
                  ...
                  <div role="alert" className="flex items-center gap-2 text-xs text-amber-300 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    <AlertCircle size={12} />
                    {uploadError ?? contextWarning}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="px-4 pb-4 pt-2 border-t border-neutral-800 shrink-0">
              <div className="flex items-end gap-2">
                ...
                <div className="flex flex-col gap-1.5 pb-1">
                  ...
                  <button
                    type="button"
                    aria-label="Upload PDF or image to advisor sources"
                  ...
                  <button
                    type="button"
                    aria-label="Attach screenshot to advisor question"
                ...
                <div className="flex-1 relative">
                  ...
                  <button
                    ...
                    aria-label="Send question to counselling advisor"
