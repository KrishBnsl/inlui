...
class JoSAAPredictor:
    ...
    def load_artifacts(self):
        ...
        model_dict = joblib.load(self.models_dir / "model_artifacts" / "final_regression_model.joblib")
        self.model = model_dict["model"] if isinstance(model_dict, dict) else model_dict
        self.prep_pipeline = joblib.load(self.models_dir / "model_artifacts" / "preprocessing_pipeline.joblib")
        
        with open(self.models_dir / "model_artifacts" / "label_encoders.json", "r") as f:
    ...
    def _generate_explanation(self, row, student_rank: int) -> str:
        ...
        if prob >= 0.8:
            ...
            cls = "moderate"
            reason = f"your rank is close to the expected boundary. Volatility could swing the cutoff either way."
        else:
            cls = "ambitious"
...
def main():
    ...
    parser.add_argument("profile", type=str, help="JSON string of student profile")
    ...
    t0 = time.time()
    out = predictor.predict(profile)
    t1 = time.time()
    
    out["inference_time_ms"] = round((t1 - t0) * 1000, 2)
