...
| Seed                 | 42                                                                         |
| Code state           | Source snapshot `6bdcac995ebab6c05bb88f7c31b4f00abd767d41`; clean worktree recorded |
| Source of truth      | [`research/reports/metrics.json`](research/reports/metrics.json)           |

The canonical JSON records `source_commit`, `generated_from_commit`, a clean
source-worktree flag, source-file hashes, and hashes for every versioned output.
The evidence commit attaches those outputs without trying to embed its own SHA.

### Strict generated figures

Only figures generated by the strict evaluator are embedded below. Obsolete
same-season plots and row-level outputs were removed from the current tree;
small invalidation sentinels remain under `research/reports/legacy/`.
...
| README contract     | Paths, links, ports, endpoints, metrics, citation       | `python scripts/verify_readme.py`                                       | PASSED               |
