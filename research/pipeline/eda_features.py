...
Reads the Day 1 master dataset, produces:
  • EDA summary report       → research/reports/legacy/eda_summary.md
  • Legacy charts → research/reports/legacy/figures/
  • Feature-engineered dataset → research/data/features/feature_engineered.csv
  • Regression splits → research/data/features/regression_*.csv
  • Classification splits → research/data/features/classification_*.csv
  • Legacy encoders and mappings → research/artifacts/legacy/
...
def run_day2_pipeline(data_dir: Path) -> None:
    ...
    reports_dir = RESEARCH_ROOT / "reports" / "legacy"
    figures_dir = reports_dir / "figures"
    artifact_dir = RESEARCH_ROOT / "artifacts" / "legacy"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory containing the processed data (default: research/data)",
    )
