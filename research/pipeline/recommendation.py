...
if __name__ == "__main__":
    raise SystemExit(
        "BLOCKED: recommendation.py uses the invalidated model and final-test "
        "residual uncertainty. Use the inference service with the strict artifacts "
        "generated by `python -m research.pipeline.research_evaluation`."
    )
...
def load_artifacts(artifact_dir: Path, data_dir: Path) -> Tuple[Any, Any, pd.DataFrame, pd.DataFrame, dict]:
    """Loads all necessary artifacts from Day 3 and datasets."""
    log.info("Loading model artifacts...")
    model = joblib.load(artifact_dir / "final_regression_model.joblib")
    prep_pipeline = joblib.load(artifact_dir / "preprocessing_pipeline.joblib")
    
    with open(artifact_dir / "label_encoders.json", "r") as f:
    ...
    test_preds = pd.read_csv(artifact_dir / "residuals_analysis.csv")
