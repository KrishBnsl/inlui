"""Legacy evaluation helpers; command-line execution is methodologically blocked.

The historical entry point estimated uncertainty from the final test residual
file. That is forbidden for current evidence and serving. The strict replacement
is ``research.pipeline.research_evaluation``.
"""

if __name__ == "__main__":
    raise SystemExit(
        "BLOCKED: evaluation.py derives uncertainty from final-test residuals and "
        "cannot generate current research evidence. Run `.venv/bin/python "
...
from .recommendation import run_monte_carlo
...
def perform_ablation_studies(merged_test: pd.DataFrame, std_map: pd.DataFrame, reports_dir: Path):
    ...
    md = [
        ...
        "\n### Metrics",
        f"- Top-{k} Overlap (A vs B): {overlap_AB:.3f}",
        f"- Top-{k} Overlap (A vs C): {overlap_AC:.3f}",
