...
LEGACY / INVALID FOR PRE-COUNSELLING RESEARCH.
...
Outputs:
    research/reports/legacy/regression_benchmark.csv|md     – model comparison table
    research/reports/legacy/classification_benchmark.csv|md – classification comparison
    research/reports/legacy/day3_model_report.md            – full narrative report
    research/artifacts/legacy/ legacy serving artifacts
    research/reports/legacy/figures/ legacy diagnostic figures
...
if __name__ == "__main__":
    raise SystemExit(
        "BLOCKED: train_models.py consumes same-season/observed feature rows and is "
        "not a valid pre-counselling experiment. Run `.venv/bin/python "
        "-m research.pipeline.research_evaluation --help` and follow "
...
def run_day3_pipeline(data_dir: Path) -> None:
    ...
    """
    reports_dir = REPO_ROOT / "research" / "reports" / "legacy"
    figures_dir = reports_dir / "figures"
    artifact_dir = REPO_ROOT / "research" / "artifacts" / "legacy"
...
def main() -> None:
    """Fail before the availability-invalid legacy pipeline can overwrite evidence."""
    raise SystemExit(
        "BLOCKED: train_models.py consumes same-season/observed feature rows and is "
        "not a valid pre-counselling experiment. Run `.venv/bin/python "
        "-m research.pipeline.research_evaluation --help` and follow "
