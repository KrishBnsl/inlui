...
analysis-ready CSV/Parquet outputs with local row-lineage tracking. Source
acquisition provenance and redistribution rights require a separate verified
record and are not established by this pipeline.
...
Outputs:
    ...
    data/processed/data_dictionary.txt         – column documentation
    data/processed/summary_report.txt           – cleaning summary
...
def add_percentile_ranks(df: pd.DataFrame, dynamics: pd.DataFrame) -> pd.DataFrame:
    ...
    This mirrors the formula in optional/rust-sim's normalization.rs.
    """
    if dynamics.empty:
...
def generate_data_dictionary(df: pd.DataFrame, output_dir: Path) -> None:
    """Generate a plain-text data dictionary documenting every column."""
    ...
    path = output_dir / "data_dictionary.txt"
    path.write_text("\n".join(lines), encoding="utf-8")
    log.info("Wrote data_dictionary.txt")
...
def generate_summary_report(
    ...
    """Generate a plain-text summary report of the cleaning process."""
    lines = [
    ...
    lines.extend([
        ...
        "",
        "The `branch_timeseries.parquet` is directly usable as input for the optional "
        "Rust simulator's "
    ...
    path.write_text("\n".join(lines), encoding="utf-8")
    log.info("Wrote summary_report.txt")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory containing the raw CSV files (default: research/data)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
